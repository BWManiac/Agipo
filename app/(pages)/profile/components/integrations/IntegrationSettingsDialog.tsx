  ...
  return (
    <div
      className={cn(
        ...
        variant === "error" && value > 0 ? "border-red-100 bg-red-50/50" : "border-slate-200"
      )}
    >
      <div className={cn("text-xs font-medium uppercase", variant === "error" && value > 0 ? "text-red-600" : "text-slate-500")}>
        {label}
      </div>
      <div className={cn("text-2xl font-semibold mt-1", variant === "success" ? "text-green-600" : variant === "error" && value > 0 ? "text-red-700" : "text-slate-900")}>
        {value}
      </div>
    </div>
  );
...
export function IntegrationSettingsDialog({ open, onOpenChange }: IntegrationSettingsDialogProps) {
  ...
  // Filter auth configs based on search
            ...
            <div className="flex justify-between items-end mb-6">
              <div>
                ...
                <p className="text-sm text-slate-500 mt-1">
                  Manage API access, auth types, and OAuth scopes for your organization.
                </p>
              </div>
              <div className="flex gap-3">
                <Button variant="outline" onClick={() => alert("Audit logs coming soon...")}>
                  View Audit Logs
                </Button>
            ...
            <div className="flex-1 bg-white border border-slate-200 rounded-lg shadow-sm overflow-hidden flex flex-col">
              <div className="p-4 border-b border-slate-200 bg-slate-50 flex justify-between items-center">
                ...
                <div className="text-xs text-slate-500">
                  {filteredConfigs.length} integration{filteredConfigs.length !== 1 ? "s" : ""}
                </div>
              </div>
