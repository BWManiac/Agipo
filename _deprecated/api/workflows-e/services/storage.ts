...
const WORKFLOWS_DIR = path.join(process.cwd(), "_tables", "workflows-e");
...
export async function createWorkflow(name?: string): Promise<WorkflowDefinition> {
  ...
  // Import createEmptyWorkflow from types (re-exported from base workflows)
  const { createEmptyWorkflow } = await import("@/app/api/workflows/services/types");
