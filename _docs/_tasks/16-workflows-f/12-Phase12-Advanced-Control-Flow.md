# Phase 12: Advanced Control Flow Transpilation

**Status:** 📋 Planned  
**Depends On:** Phase 8 (Transpilation Engine), Phase 9 (Workflow Inputs Enhancement), Phase 10 (Agent Integration), Phase 11 (Workflow Runtime Execution)  
**Started:** TBD  
**Completed:** TBD
...
| # | Criterion | Test | Part |
|---|-----------|------|------|
| AC-12.1 | Branch steps produce `.branch()` code | Branch node → conditional routing code | A |
| AC-12.2 | Branch conditions reference step outputs and inputs | Uses `getStepResult()` for step outputs and `inputData` for workflow inputs | A |
| AC-12.3 | Parallel groups produce `.parallel()` code | Parallel container → `[step1, step2]` array | A |
| AC-12.4 | Loop steps produce `.dountil()` code | Loop container → repeat with condition | B |
| AC-12.5 | ForEach steps produce `.foreach()` code | ForEach container → iteration code | B |
| AC-12.6 | Nested control flow generates correctly | Loop inside branch, foreach inside parallel lane, branch inside loop → all generate valid nested code | B |
| AC-12.7 | Control flow preserves data mappings | Mappings work inside containers | A, B |
...
| # | Criterion | Test |
|---|-----------|------|
| AC-12.1 | Branch steps produce `.branch()` code | Branch node generates conditional |
| AC-12.2 | Branch conditions reference step outputs | Uses getStepResult |
| AC-12.3 | Parallel groups produce `.parallel()` code | Parallel container works |
| AC-12.7 | Control flow preserves data mappings | Mappings in branches work |
...
| # | Criterion | Test |
|---|-----------|------|
| AC-12.4 | Loop steps produce `.dountil()` code | Loop container generates |
| AC-12.5 | ForEach steps produce `.foreach()` code | ForEach container generates |
| AC-12.6 | Nested control flow generates correctly | Nested structures work |
