...
- `workflow-executor.ts` - Execute code from database with ES→CommonJS transformation
...
**Why It's Right:**
- ✅ **Matches Mastra's design** - Workflows are objects, not files
- ✅ **No dynamic imports** - We read JSON (static file), construct in memory
- ✅ **Simple** - Direct API usage, no code generation needed
- ✅ **Type-safe** - Use Zod schemas directly, no string manipulation
- ✅ **Maintainable** - Standard patterns, easy to debug
...
### Learning 2: JSON Schema → Zod Conversion at Runtime
...
- Same pattern as connection tools: load → wrap → return `ToolDefinition`
...
┌─────────────────────────────────────────────────────────────┐
│                     User Saves Workflow                      │
│  (workflow.json written to _tables/workflows/{id}/)         │
└──────────────────────┬──────────────────────────────────────┘
                       │
                       ▼
┌─────────────────────────────────────────────────────────────┐
│              Optional: Transpiler Generates .ts             │
│  (For IDE support, debugging, version control)             │
│  NOT used at runtime - just for developer experience       │
└──────────────────────┬──────────────────────────────────────┘
                       │
                       ▼
┌─────────────────────────────────────────────────────────────┐
│              Agent Requests Workflow Tool                   │
│  (chat-service.ts calls getWorkflowToolExecutable())       │
└──────────────────────┬──────────────────────────────────────┘
                       │
                       ▼
┌─────────────────────────────────────────────────────────────┐
│         workflow-loader.ts: Read workflow.json              │
│  - Check cache (by lastModified timestamp)                 │
│  - If cached, return cached workflow object                 │
│  - If not cached, call buildWorkflowFromDefinition()       │
└──────────────────────┬──────────────────────────────────────┘
                       │
                       ▼
┌─────────────────────────────────────────────────────────────┐
│      workflow-builder.ts: Construct Workflow Object         │
│  1. Convert runtimeInputs → inputSchema (JSON Schema → Zod) │
│  2. Create workflow base: createWorkflow({ ... })           │
│  3. For each step:                                          │
│     - Convert step schemas (JSON Schema → Zod)               │
│     - Build execute function from step config               │
│     - Create step: createStep({ ... })                      │
│     - Add mapping transform if bindings exist              │
│     - Chain: workflow.then(step)                            │
│  4. Commit: workflow.commit()                              │
│  5. Return workflow object                                 │
└──────────────────────┬──────────────────────────────────────┘
                       │
                       ▼
┌─────────────────────────────────────────────────────────────┐
│         workflow-loader.ts: Cache & Return                   │
...
└──────────────────────┬──────────────────────────────────────┘
                       ...
                       ▼
┌─────────────────────────────────────────────────────────────┐
│      workflow-tools.ts: Wrap as Vercel AI SDK Tool           │
│  1. Get workflow object (from loader)                       │
│  2. Get workflow metadata (name, description)                │
│  3. Wrap in tool():                                         │
│     - inputSchema: workflow.inputSchema (already Zod)       │
│     - execute: Create RuntimeContext, run workflow          │
│  4. Return ToolDefinition with .run property               │
└──────────────────────┬──────────────────────────────────────┘
                       │
                       ▼
┌─────────────────────────────────────────────────────────────┐
│              Agent Executes Workflow Tool                   │
│  - Agent calls tool.execute(input)                          │
│  - RuntimeContext created with connections & userId         │
│  - Workflow run created: createRunAsync({ resourceId })    │
│  - Workflow started: run.start({ inputData, runtimeContext })│
│  - Steps execute with RuntimeContext access                 │
...
app/api/workflows/
├── services/
│   ├── workflow-builder.ts          ✅ Core: Construct workflows from JSON
│   ├── workflow-loader.ts            ✅ Core: Load & cache workflows
│   └── storage/
│       ├── crud.ts                   ✅ Read/write workflow.json
│       ├── utils.ts                  ✅ File path utilities
│       └── code-writer.ts            ✅ Optional: Write .ts files for IDE
│
├── [workflowId]/
│   └── update/
│       └── services/
│           ├── transpiler/           ✅ Optional: Generate .ts files for IDE
│           └── storage/
│               └── code-writer.ts    ✅ Optional: Write .ts files
│
app/api/tools/services/
└── workflow-tools.ts                 ✅ Wrap workflows as agent tools
```

### What We DON'T Need

❌ **Database storage** - Workflows are objects, not code  
❌ **Code executor** - No code to execute  
❌ **ES module transformation** - No modules to transform  
❌ **Sandboxing** - No user code to sandbox  
❌ **Module whitelisting** - No dynamic imports  
...
4. `buildMappingTransform(stepId: string, bindings: Record<string, StepBindings>)`
   - Return async function that maps:
     - `workflow-input` → `inputData[workflowInputName]`
     - `step-output` → `getNestedValue(getStepResult(sourceStepId), sourcePath)`
     - `literal` → literal value

5. `buildWorkflowFromDefinition(definition: WorkflowDefinition, bindings: Record<string, StepBindings>)`
   - Convert runtimeInputs → inputSchema (JSON Schema → Zod)
...
1. `app/api/workflows/services/workflow-builder.ts` - ✅ Core
2. `app/api/workflows/services/workflow-loader.ts` - ✅ Core
3. `app/api/tools/services/workflow-tools.ts` - ✅ Core
4. `app/api/workflows/services/storage/crud.ts` - ✅ Core
5. `app/api/workflows/services/storage/utils.ts` - ✅ Core
6. `app/api/workflows/services/storage/code-writer.ts` - ✅ Optional (IDE support)
7. `app/api/workflows/[workflowId]/update/services/transpiler/` - ✅ Optional (IDE support)
...
|---|-----------|---------------|
| AC-11.2.1 | Workflow constructed from JSON | Call `buildWorkflowFromDefinition()` → returns workflow object with `createRunAsync()` |
| AC-11.2.2 | JSON Schema → Zod conversion works | Workflow `inputSchema` is valid Zod schema |
| AC-11.2.3 | Composio steps execute correctly | Run workflow → Composio tools execute with connections from RuntimeContext |
| AC-11.2.4 | Data mappings work | Run workflow → step inputs mapped from previous steps using bindings |
| AC-11.2.5 | RuntimeContext passed correctly | Execute workflow → steps receive connections and userId via RuntimeContext |
| AC-11.2.6 | Agent can invoke workflow | Chat with agent → workflow tool appears and executes successfully |
| AC-11.2.7 | Workflow caching works | Load same workflow twice → second load uses cache |
| AC-11.2.8 | No dynamic import errors | Load workflow → no "expression is too dynamic" error |
| AC-11.2.9 | Thenable workflows handled | Workflow with `then()` method → wrapped correctly, no promise resolution issues |
| AC-11.2.10 | Workflow execution succeeds | Agent invokes workflow → completes successfully with correct results |
...
2. ⏳ **Delete unused files** (database.ts, test scripts, outdated docs)
3. ⏳ **Clean rebuild** (implement the 3 core files from scratch)
4. ⏳ **Test** (verify all acceptance criteria)
5. ⏳ **Update documentation** (new README describing runtime construction)
